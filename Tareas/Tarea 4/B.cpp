...
struct SegmentTree{
  ...
  T merge(T a, T b){ 
    // Reemplazar esta funcion para determinar como juntar dos nodos para obtener el valor de su padre, en este caso es suma
    // aqui tiene que hacer mcd
    // if 
    return gcd(a,b);
  }
...
int main(){
    ...
    vector <int> a(n);
    for(int i=0; i<n;i++){
        cin >> a[i]; // lleno la cosa
    }
    SegmentTree<int> st(a); // arbol
    int c;
