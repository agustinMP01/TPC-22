...
struct SegmentTree{
  ...
  SegmentTree(vector<T> &init) { 
    ...
    build(1, 0, N-1, init); //Construyo el segment tree desde el nodo 0, en [0,N-1] con el arreglo init
  }
  ...
  T query(int l, int r){
    ...
    return query(1, 0, N-1, l, r); //Llama desde la raiz, mira todo el arreglo [0, N-1] y quiero buscar en [l, r]
  }
  ...
  void update(int t, T val){
    ...
    update(1, 0, N-1, t, val); //Llama en el nodo 0, viendo el intervalo [0, N-1], busca el indice t y el valor
  }
  ...
  int search(int from, T val){
    ...
    return search(1, 0, N-1, val+query(0, from-1));
  }
